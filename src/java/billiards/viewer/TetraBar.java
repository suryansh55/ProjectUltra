 ...
 * Opens a new window that allows the input of the (x, y) coordinates of a list
 * of point, and calculates the
 * tetrahedrons created from each point.
 * Updated May 16, 2025.
 * Allows the input of multiple coordinates. The x and y values of each
 * coordinate should be in the same line and
 * separated by a singular whitespace character. Different coordinates should be
 * separated by a newline character.
 * Updated May 20, 2025.
 * Added new fields for how many of the results to print, and whether we are
 * calculating a Bar or Tetrahedron.
 * Updated Jun 4, 2025.
 * When this window is opened, can close main window without causing a
 * exception.
...
public final class TetraBar {
	...
	// Just keep tetrahedron points for all the coordinates as a list of tuples.
	// Every three consecutive points belong
	...
	public TetraBar(Stage parentStage, Viewer viewer) {
		if (coordsDefault.isEmpty())
			coordsDefault = Utils.readFromFile("tetrahedron.txt");
		...
		final HBox hbox = new HBox(10, epsTextField, printCountTextField, tetraRadio, barRadio, drawCheckBox,
				addToCoverCheckBox, loadButton);
		...
		// Allow the window to be hidden behind the main window while preventing the
		// IllegalStateException by closing this stage when the parentStage is closed
		parentStage.setOnCloseRequest(event -> {
			stage.close();
		});
		...
		loadButton.setOnAction(event -> {
			...
			for (String coord : coords) {
				if (coord.startsWith("//") || coord.trim().isEmpty())
					continue;
	...
	private HBox getBottomHBox() {
		...
		return new HBox(10, backwardButton, lineNumTextField, goToLineButton, forwardButton, addToAllPositiveCB,
				addToPlusMinusCB);
	}
	 ...
	 * <b>Jul 2, 2025</b>
	 * <p>
	 * Extracts the integer entered in <code>textField</code>. Throws a
	 * <code>NumberFormatException</code> if what
	 ...
	 * <b>Jun 29, 2025</b>
	 * <p>
	 * Gets the user entered line number. Defaults to 1 if the user did not enter
	 * anything.
	 * </p>
	 * 
	 * @return The line number entered by the user, or 1 if the user did not enter
	 *         anything.
	 */
	 ...
	 * <b>Jun 29, 2025</b>
	 * <p>
	 * Sets the line number text field and the internal state to
