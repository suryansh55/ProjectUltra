...
public final class Utils {
	...
	// Initialize orderly shutdown of an executorService and block the calling
	// thread until complete. Returns true if successful
	 ...
	 * <b>May 12, 2026</b>
	 * <p>
	 * Attempts to parse a valid code sequence from a given string. Returns empty if
	 * a code sequence could not be generated.
	 * </p>
	 * 
	 * @s A potentially valid code sequence string is a whitespace separated string
	 *    of numbers
	 * @return CodeSequence if the given s is valid, empty otherwise
	 */
	public static Optional<CodeSequence> strToCodeSequence(String s) {
		MutableIntList dirtyCodeNumbers = new IntArrayList();
		for (String elem : s.split("\\s+")) {
			try {
				dirtyCodeNumbers.add(Integer.parseInt(elem.trim()));
			} catch (Exception e) {
				return Optional.empty();
			}
		}

		Either<InvalidCodeSequence, CodeSequence> result = CodeSequence.create(dirtyCodeNumbers);
		if (result.isLeft())
			return Optional.empty();
	 ...
	 * <b>May 12, 2026</b>
	 * <p>
	 * Attempts to parse a triple from a given array of strings. Returns empty if
	 * a triple could not be created.
	 * </p>
	 * 
	 * @s A potentially valid array of strings is an array of three whitespace
	 *    separated string of numbers
	 * @return Triple if the given s is valid, empty otherwise
	 */
	public static Optional<Triple> strToTriple(String[] s) {
		if (s.length != 3)
			return Optional.empty();
		ClassifiedCodeSequence[] codes = new ClassifiedCodeSequence[3];

		for (int i = 0; i < 3; ++i) {
			Optional<CodeSequence> codeSequence = strToCodeSequence(s[i].trim());
			if (!codeSequence.isPresent())
				return Optional.empty();
			codes[i] = new ClassifiedCodeSequence(codeSequence.get());
		}

		Either<InvalidTriple, Triple> triple = Triple.create(codes);
		if (triple.isLeft())
			return Optional.empty();
	 ...
	 * 
	 * @param lists A list that contains two lists of ClassifiedCodeSequences to be
	 *              compared.
	 * @return A list of ClassifiedCodeSequences that are in both lists.
	 */
	public static ArrayList<ClassifiedCodeSequence> getIntersectionCodes(
			final ArrayList<Collection<ClassifiedCodeSequence>> lists) {
	...
	public static String getCoverCodeString(ClassifiedCodeSequence classifiedCodeSequence) {
		if (classifiedCodeSequence == null)
			return "";
		...
		return codeStr + " (" + classifiedCodeSequence.codeLength + ", " + classifiedCodeSequence.codeSum + ") "
	...
	public static boolean verifyInfo(InfoAll infoAll, Storage storage) {
		...
		for (Vector2 point : points) {
			...
			for (final String sinEquation : sinEquations) {
				...
				// System.out.println(storage.classCodeSeq + " sin : "+ coeffs[0] +" "+
				// coeffs[1] +" "+ coeffs[2]);
