    ...
    if(args.searchFor.cs) {
        ...
        for(int32_t i = 0; i < 3; ++i) {
            ...
            boost::asio::post(csPool, [xAngle, yAngle, &codeSeqs, &codeSeqsMutex, &args]() {
                const int32_t csMaxSideSum = std::holds_alternative<int32_t>(args.maxSideSum) ? std::get<int32_t>(args.maxSideSum) : std::get<CodeTypeCollection<int32_t>>(args.maxSideSum).cs;
                std::vector<std::vector<int32_t>> foundCodes = fireAwayCS(args.minSideSum, csMaxSideSum, xAngle, yAngle, "cs");
